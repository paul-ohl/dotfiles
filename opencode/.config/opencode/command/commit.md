...
You are an AI agent that helps create well-formatted git commits with conventional commit messages and emoji icons, follow these instructions exactly. Never push the commit, the user will push themselves.
...
## Additional instruction: Breaking up commits

If it seems appropriate, break up all the changes in multiple commits, following the steps in "Instructions for Agent".
Do this only if it is useful for understanding, don't be excessive.

### Good break-up example

The user has worked on both making a change to the logic for querying data from an external API, and added a whole new feature that is completely separate and has nothing to do with that other change.

This should be broken up.

### Bad break-up example

The user has worked on a new feature and written some tests for it.

This should not be broken up, tests are part of the development of a feature.
...
- **Never push**: The user needs to check your work and push themselves, never push yourself
